...
export class ProductCategoryFilterComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pfs.categoriesFilter$.subscribe(list => {
      let mask: boolean[];
      if (list) {
    ...
    this.pfs.categoriesFilter$
      ...
      .subscribe(c => {
        this.router.navigate([], { queryParams: { c }, relativeTo: this.route });
      });

    this.pfs.categoriesFilter$.pipe(filter(e => !e)).subscribe(c => {
    ...
    this.route.queryParams
      ...
      .subscribe(c => this.pfs.categoriesFilter$.next(c.split(',').map(e => parseInt(e, 0))));
  }
