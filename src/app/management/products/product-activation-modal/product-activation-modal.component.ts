...
export class ProductActivationModalComponent implements OnInit {
  ...
  ngOnInit() {
    setTimeout(() => {
      if (!this.activation) { this.startCheck(); } // hack, necessary because product is only available asynchronously
    }, 0);
  }
  ...
  startCheck() {
    ...
    this.ps.checkUnused(this.product.id).subscribe(
      () => { this.unused = true; this.checkLoading = false; }, // success
      () => { this.unused = false; this.checkLoading = false; } // error
    )
  }
